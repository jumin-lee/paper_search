...
- 중요하지 않은 논문은 각 행의 **✕ 버튼**으로 메인 피드에서 치울 수 있습니다.
- 제외한 논문은 **"제외" 탭**에서 모아 보고, ↩ 버튼으로 복원할 수 있습니다.
- 제외해도 논문 데이터는 `papers.js`에 그대로 남으므로 **수집기가 같은 논문을
  다시 가져오지 않습니다** (행을 삭제하면 다음 수집 때 재유입되니 ✕를 쓰세요).
- 기기 간 공유는 북마크와 동일: **"제외 내보내기"** → `data/hidden.js` 덮어쓰고 커밋.
- `window.EXCLUDED`(papers.js 하단)는 별개로, 봇이 **애초에 추가하지 않을** 논문 목록입니다.

## 자동 수집 논문의 평가방법 분류

- 학회(월간)·arXiv(일간) 수집 모두 abstract 내용을 키워드 매칭해 평가방법을 **추정**해서 채웁니다
...
- note 표시 구분: `eval abstract 확인`(초록에 평가방법 명시) / `eval 추정 ⚠️`(정황 추정) /
  `eval 자동추정`(키워드 매칭) — 자동추정은 정확하지 않을 수 있으니 중요한 논문은 확인 후 수정하세요.
